...
| 状態ストレージ容量 | 約 50 PB（意識データ10,240人分 × 2.4 TB + メタデータ + ゲーム状態） |
...
| 署名者ID | 氏名 | 所属 | 役職 | 備考 |
|----------|------|------|------|------|
| `GOV-SIG-01` | ヴィクター・ラング | NH社 | CEO | `CHAR-020` |
...
| 署名者 | 署名可否 | 理由 | 詳細 |
|--------|---------|------|------|
| `GOV-SIG-01` ラング | ❌ **不可** | **法的差押え** | 事件発生後、米国SEC（証券取引委員会に相当する架空機関：Federal Digital Assets Commission, FDAC）がNH社CEO個人の暗号鍵を含む全デジタル資産を差し押さえ。HSMは連邦裁判所の証拠保全室に移送済み。裁判所命令なしに使用不可 |
...
| 想定される反論 | 回答 |
|--------------|------|
| 「サーバーの電源を物理的に切れば？」 | `RULE-008`: 意識格納中の電源喪失は人格データの破損を引き起こす。10,240人の人格が消滅するリスクがあり、誰も責任を取れない |
| 「裁判所にラングの鍵を使わせろと命令すれば？」 | FDACの差押えは連邦レベル。解除には別の裁判手続きが必要で**数週間〜数ヶ月**かかる。さらにFDACは「証拠としての鍵を使用すると証拠能力が毀損される」と主張 |
