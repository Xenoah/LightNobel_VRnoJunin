...
| ID | 用語 | 英語 | 分類 | 定義 | 参照 |
|----|------|------|------|------|------|
| `GLOSS-001` | アーク (Arc) | Arc | 通貨 | Arcadia内の基本通貨単位。最小単位は mArc（ミリアーク） | `RULE-040` |
| `GLOSS-002` | アルカディア | Arcadia | 固有名 | NH社が開発したメタバースプラットフォーム。VR世界の総称 | `TECH-002` |
...
| ID | 用語 | 英語 | 分類 | 定義 | 参照 |
|----|------|------|------|------|------|
| `GLOSS-010` | 差分同期（δ同期） | Delta Sync | 技術 | NCBの意識状態更新方式。前回からの差分のみ送信 | `TECH-005` |
| `GLOSS-011` | セトラーズ | Settlers | 派閥 | 定住派。VR内で永続的社会を構築する派閥 | `FACT-002` |
| `GLOSS-012` | 聖域 | Sanctuary | 場所 | Ethereal Gardens最高島にあるThe Awakened本部 | `LOC-003` |
| `GLOSS-013` | スナップショット | Snapshot | 技術 | 意識状態の全体記録。512δごとに統合生成 | `TECH-007` |
...
| ID | 用語 | 英語 | 分類 | 定義 | 参照 |
|----|------|------|------|------|------|
| `GLOSS-016` | ネクサス認知エンジン (NCE) | Nexus Cognitive Engine | 技術 | Arcadia内蔵のLLM。NPC対話・推論・ログ分析に使用 | `TECH-008` |
| `GLOSS-017` | ネクスフロート | NexFloat | 技術 | カプセル充填用フッ素系不活性液体。比重1.02 | `TECH-003` |
| `GLOSS-018` | ネクスニュートリ | NexNutri | 技術 | カプセル用液体栄養剤。2,000 kcal/日 | `TECH-004` |
...
| `GLOSS-026` | ファントムVR | Phantom VR | 医学 | 帰還後にVR内感覚が残像として知覚される症状 | `GOV-005` |
| `GLOSS-027` | 保守金庫 | Maintenance Vault | 場所 | The Undercroft最深部の旧デバッグコンソール | `LOC-002` |

## ま行

| ID | 用語 | 英語 | 分類 | 定義 | 参照 |
|----|------|------|------|------|------|
| `GLOSS-028` | マルチシグ | Multi-Signature | 技術 | 複数の署名者による合意で実行される暗号学的承認方式 | `GOV-002` |
| `GLOSS-029` | メンテナンスオーバーライドホルト | Maintenance Override Halt | 技術 | 保守用の代替停止コマンド。バリデーター14ノード確認が必要 | `GOV-005` |

## ら行

| ID | 用語 | 英語 | 分類 | 定義 | 参照 |
|----|------|------|------|------|------|
| `GLOSS-030` | レーテ・チェーン | Lethe Chain | 技術 | Arcadiaの基盤ブロックチェーン | `GOV-001` |
| `GLOSS-031` | ロイヤリスト | Loyalists | 派閥 | 運営擁護派 | `FACT-005` |
...
| `GLOSS-036` | HSM | Hardware Security Module | 技術 | 暗号鍵を安全に保管する専用ハードウェア | `GOV-002` |
| `GLOSS-037` | FDAC | Federal Digital Assets Commission | 組織 | 米国のデジタル資産規制機関（架空） | `GOV-003` |
...
## テスタースラング（VR内で自然発生した言葉）

| スラング | 意味 | 用例 |
|---------|------|------|
| 「肉体（ミート）」 | 現実世界の身体 | 「ミートの方は大丈夫なのかな……」 |
| 「チェーンド」 | 意識がLethe Chainに囚われている状態 | 「俺たち全員チェーンドだよ」 |
| 「ゴースト」 | 意識の断片化を経験した人 | 「あいつ、ゴーストになりかけてる」 |
| 「フリーズ」 | 帰還（停止コマンドの実行結果） | 「フリーズはいつ来るんだ？」 |
| 「NFC落ち」 | ニュースによって精神的に参ること | 「また新しいニュースで全員NFC落ちしてる」 |
