...
| 状態ストレージ容量 | 約 180 TB（意識データ48人分 × 2.4 TB + メタデータ + ゲーム状態） |
| スマートコントラクト言語 | Solidity（Lethe Chain向け拡張版） |
...
| 必要署名数 | **5-of-7** |
| 実行結果 | 全テスターの意識データの「安全停止シーケンス」起動 → 脳への逆変換準備 → カプセル覚醒プロセス開始 |
...
| `GOV-SIG-04` コーエン | ❌ **不可** | **鍵の物理的喪失** | HSMがイスラエルの本社サーバールームにあったが、事件直後の混乱で施設がサイバー攻撃を受け、HSMのファームウェアが破壊された。鍵のバックアップは存在しない（セキュリティポリシーで禁止）。**【真相】このサイバー攻撃はAEGISが組織的に実行。鍵を「破壊」すると見せかけて、意識データの断片的コピーも密かに実行した可能性あり** |
| `GOV-SIG-05` ウィンストン | ❌ **不可** | **署名拒否（倫理的理由）** | 「停止シーケンスが意識データを破損させるリスクがゼロでない限り署名しない」と主張。安全性の完全な証明を要求しているが、証明には実際に停止してみるしかない（循環論法） |
| `GOV-SIG-06` アルサード | ❌ **不可** | **内紛／利益相反** | Atlas Infrastructure Partners内で株主間紛争が発生。アルサード個人は署名意思があるが、取締役会が「訴訟リスク回避のため署名を禁止」する決議を可決。個人の判断で署名すると背任で告訴される。**【真相】Atlas社の大株主の一人がAEGISの関連投資ファンドであり、取締役会決議はその圧力で可決された** |
...
> **RULE-082**: 4名の署名不能のうち、**2名**（コーエン、アルサード）はAEGISの工作によるもの。残り2名（クロス、ウィンストン）は純粋な偶然。「偶然に乗じた陰謀」がリアリティを維持する核心設定。
...
| 想定される反論 | 回答 |
|--------------|------|
| 「サーバーの電源を物理的に切れば？」 | `RULE-008`: 意識格納中の電源喪失は人格データの破損を引き起こす。48人の人格が消滅するリスクがあり、誰も責任を取れない |
...
| Phase 3: 逆変換 | 2〜5時間 | 意識データを脳へ書き戻し（並列処理、カプセルの神経I/Oを使用） |
...
| **意識データ劣化による記憶欠損** | **約3%（3名）** | **高（`TECH-012`参照。Pixelが最重症）** |
| **帰還拨絶反応による意識消滅** | **2名（The Awakened信者）** | **致命（脳死）** |
...
- **OQ-GOV-003**: AEGISのコーエンHSM攻撃時に意識データの断片をコピーしたという設定の技術的整合性
  - HSM攻撃とLethe Chainへのアクセスは異なるレイヤー。コピーは「HSM攻撃と同時に実行された別のオペレーション」として整理
  - Vanguard社のノード（`GOV-NODE-04`）経由で、HSMファームウェア破壊前の専柉にLethe Chain上の意識データの一部を抽出
  - **採用**: 完全な意識コピーではなく「断片的人格パターン」。エピローグの「デジタルゴースト」暗示に接続
