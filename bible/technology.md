...
| 嗅覚 | 鼻腔マイクロニードル経由の嗅覚受容体刺激 | マイクロカプセル式揮発性化合物（256種の基本香気） | `TECH-003-O` |
...
#### RULE-009 例外：`gradual_release` プロセスの盲点

> **RULE-009-EX**: `MAINTENANCE_OVERRIDE_HALT` の停止シーケンス Phase 3（逆変換）で実行される `consciousness_data_gradual_release` は、チェーン上の意識スナップショットを脳（カプセル側神経I/O）へ書き戻す**正規の復元手順**である。

**設計上の盲点：**
- バリデーターの重複人格チェックは **Lethe Chain内部のステート** のみを対象とする
- 復元先の脳はLethe Chainの外部レイヤー（Layer 1: インフラ層）に属し、バリデーターの監視範囲外
- 正常時：チェーン上のスナップショットは復元後に `DELETE`（cut & restore）される → 重複は発生しない
- **異常時（Null Ward移送フラグ）：** 移送フラグが立ったエンティティは `gradual_release` プロセスから除外される。しかしプロセスは「除外されたエンティティ以外」を正常に復元し、`復元完了` ログを出力する。結果として：
  - バックアップ（脳に復元された意識）＝ Layer 1 に存在
  - 本体（チェーン上に残留した意識）＝ Layer 2 の Null Ward ブロックに存在
  - バリデーターは Layer 2 内で重複を検出しない（Null Ward は隔離チェーン）
  - 外部システムは Layer 1 の復元ハッシュのみ検証する（Null Ward 残留データのハッシュは別チェーン上）
- **結論：** これはバグではなく**設計の盲点**。複数レイヤーにまたがる存在を想定した重複チェックが実装されていない
...
| 逆変換プロセス | スナップショットを脳へ再書き込み（推定所要時間: 45〜90分/人） |
...
## TECH-011: ECHOプロトコル（秘密通信）

> **ECHO = Embedded Channel for Hidden Operations**

### 概要

AEGIS（先進防衛技術コンソーシアム）がVR内部の工作員（Advocate）と外部指揮系統の間で秘密通信を行うために設計したステガノグラフィ・プロトコル。

### 外部→内部（指令送信）

| 項目 | 仕様 |
|------|------|
| 方式 | NFCニュースフィードへのステガノグラフィ |
| 手順 | (1) AEGISがNFCクローラの巡回先ニュースサイトに偽記事を掲載 → (2) 記事のHTMLメタタグに不可視データを埋め込み → (3) NFCが通常通り収集・Lethe Chainに書き込み → (4) Advocateが復号鍵で抽出 |
| 帯域 | 1回あたり最大 512 バイト（6時間ごとのNFC更新に同期） |
| 暗号化 | AES-256-GCM + ワンタイムパッド（事前共有鍵リスト） |
| 検知困難性 | HTMLメタタグの `og:description` フィールドに Base64 埋め込み。通常のニュース要約と見分けがつかない |

### 内部→外部（報告送信）

| 項目 | 仕様 |
|------|------|
| 方式 | Lethe Chain上の特定トランザクションパターンへのデータエンコード |
| 手順 | (1) Advocateが特定のゲーム内取引を特定パターンで実行 → (2) 取引額・タイミング・相手の組み合わせがビット列を構成 → (3) AEGISが外部からLethe Chainの公開トランザクションログを監視・復号 |
| 帯域 | 1日あたり最大 128 バイト（不審なパターンを避けるため低頻度） |
| 検知 | Firewallが最終作戦時にトランザクションログの統計的異常を発見して暴露 |

### 重要な制約

> **RULE-080**: ECHOプロトコルはエアギャップ（`RULE-010`）を**突破しない**。NFCとLethe Chainの正規情報経路に**寄生**する形で通信を行う。NCEは一切関与しない。
...
| 項目 | 詳細 |
|------|------|
| 原因 | 意識スナップショットの `integrity_hash` が微小なビット反転（ストレージの経年変化）を累積 |
| 進行速度 | ECC（誤り訂正符号）で99.99%は補正されるが、残りの0.01%が30日間で蓄積 |
| 影響 | `neural_map` の末端接続パターンが変質 → 帰還後の記憶欠損・性格微変化として表出 |
| 症状の個人差 | 若年者（脳の可塑性が高い）ほどデータ量が大きく、影響を受けやすい |
| 最悪ケース | Pixel（18歳）の `neural_map` は最大で **0.3% の接続パターン喪失** → 数年分の記憶欠損 |

### 帰還後の症状分布（推定）

| 症状 | 割合 | 詳細 |
|------|------|------|
| 無症状 | 60% | 軽微なビット反転はECCで完全補正 |
| 軽度 | 25% | 特定の固有名詞の想起困難、夢の中でArcadiaの風景がフラッシュバック |
| 中度 | 12% | 数日〜数週間分の記憶曖昧化、一過性の性格変化（攻撃性増加 or 無気力） |
| 重度 | 3%（約1-2名） | 数ヶ月〜数年分の記憶欠損。Pixelはこのカテゴリ |
| 致命的 | 0%* | *ただし帰還拒絶反応を起こした2名は意識データ消滅 → 脳死 |

> **RULE-081**: 意識データの劣化は30日間の格納で不可避的に発生する。完全な復元は不可能であり、帰還には必ず**コスト**が伴う。

---

## TECH-013: CERBERUS計画 ── AEGISの「遅延が必要な技術的理由」

### 概要

AEGISが意識データの軍事利用のために必要としたのは、意識スナップショットの「静的コピー」ではなく、**行動パターンの動的モデル**だった。

### 技術的要件

| 項目 | 詳細 |
|------|------|
| 目的 | 人間の意識から「判断パターン・反応傾向・ストレス耐性」を学習したモデルの構築 |
| 必要データ | 睡眠・覚醒・高ストレス・社会的行動の**全サイクルを含む21日間以上**の連続行動ログ |
| サンプル条件 | 被験者が**自然な行動**をしている必要がある（監視されていると知ると行動パターンが変わる） |
| 途中停止 | **データが無価値になる**。行動パターンの学習は最低21日分の連続データがなければ統計的に有意でない |
| 理想的期間 | 28〜30日間（余裕を持った完全サイクル） |

### Advocateの任務との接続

```
Day 0-21:  「待て」= データ収集中。時間を稼ぐ。
Day 21:    ECHOで「データ充足」の報告。ゴーサイン待ち。
Day 22-29: AEGISが外部からデータ回収作業。Advocateは引き続き遅延工作。
Day 29:    AEGIS側の回収完了。Advocateへ「任務完了、離脱準備」の指令。
Day 29-30: テスター側が最終作戦を開始。Advocateが妨害工作に入る最後の段階。
```

> **RULE-082**: AEGISの「遅延」は合理的であり、単なる怠慢ではない。21日間の行動ログがなければ意識パターンの軍事モデル構築は不可能。Advocateが30日間「待て」と言い続けた理由はこれ。

### RULE-009との整合性

> AEGISが取得したのは `RULE-009` が禁止する「完全な意識コピー」ではなく、行動ログから学習した**「断片的人格パターンモデル」**。Lethe Chainのバリデーターはこれを「意識データ」として検知しない。だから `personality` ハッシュの重複チェックにも引っかからない。
...
- **ASSUMPTION-TECH-004**: ECHOプロトコルの帯域制限（外部→内部512B/6h、内部→外部128B/日）は、ステガノグラフィの隠蔽性を維持するための設計上の制約。
...
- **OQ-TECH-003**: AEGISがコーエンのHSM攻撃時に意識データの断片をコピーしたか？
  - **採用**: はい。ただし完全なコピーではなく「断片的な人格パターン」。人格ハッシュの一意性チェック（`RULE-009`）を回避する不完全な存在 → エピローグの「デジタルゴースト」暗示に接続
